...
mod tests {
    ...
    use crate::{ScopeKind, SemanticContext};
    ...
    mod type_interner_contracts {
        ...
        fn interned_types_retrievable() {
            ...
            let ref_i32 = interner.mk_ref(crate::Mutability::Shared, interner.i32());
    ...
    // =========================================================================
    // Bounds Checking Contracts (debug_assert! verification)
    //
    // These tests document and verify that invalid compiler-internal IDs
    // correctly trigger debug_assert! panics. This is expected behavior:
    // these IDs are generated by the compiler, not user input, so invalid
    // IDs indicate a compiler bug.
    // =========================================================================

    mod bounds_checking_contracts {
        use super::*;
    ...
    // =========================================================================
    // User-Facing Bounds Checks (Diagnostic verification)
    //
    // These tests verify that out-of-bounds access from user code produces
    // helpful diagnostics rather than panics. This is the correct behavior
    // for user errors (as opposed to compiler bugs).
    // =========================================================================
    ...
    mod user_facing_bounds_checks {
        ...
        ///
        /// When user code accesses `tuple.5` on a 3-element tuple, the compiler
        /// should produce a helpful error message, not crash.
        #[test]
        fn tuple_out_of_bounds_emits_diagnostic() {
            let source = r#"
                fn test() {
                    let t = (1, 2, 3);
                    let x = t.5;
            ...
            assert!(
                infer_result
                    ...
                    .any(|d| d.message.contains("out of range")
                        ...
                        || d.message.contains("does not have")
                        || d.message.contains("field access on non-struct")),
                "expected out-of-bounds diagnostic, got: {:?}",
                infer_result
                    .diagnostics
                    .iter()
                    .map(|d| &d.message)
        ...
        /// Out-of-bounds tuple field on 2-element tuple.
        #[test]
        fn tuple_index_2_on_2_element_emits_diagnostic() {
            let source = r#"
                fn test() {
                    let t = (1, 2);
                    let x = t.2;
        ...
        /// Empty tuple field access emits diagnostic.
        #[test]
        fn empty_tuple_field_access_emits_diagnostic() {
            let source = r#"
                fn test() {
                    let t = ();
                    let x = t.0;
            ...
            assert!(
                ...
                "expected diagnostic for accessing field 0 on unit type"
            );
        }

        /// Valid tuple access does not emit diagnostic.
        #[test]
        fn valid_tuple_access_no_diagnostic() {
            let source = r#"
                fn test() {
                    let t = (1, 2, 3);
                    let x = t.0;
            ...
            assert!(
                ...
                "unexpected tuple access errors: {:?}",
                tuple_errors.iter().map(|d| &d.message).collect::<Vec<_>>()
            );
        }
    }
}
