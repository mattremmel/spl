...
This document specifies iteration mechanisms in SPL, including `for` loops, the `IndexIterator` trait, internal iteration methods, and generators.

## Overview

SPL's iteration model is built on **intersection semantics**: functions can return references that are assumed to borrow from all input references. This enables traits like `IndexIterator` to have methods that return references to collection elements.

- **`for` loops** use the `IndexIterator` trait for reference iteration over indexed collections
...
|-------|---------|--------------|
| `IndexIterator` | Reference iteration over indexed collections | `get(&self, i): &T` creates fresh borrow each call |
...
The compiler transforms `for` loops into method calls on the `IndexIterator` trait:
...
### Desugaring: IndexIterator vs Iterator

The `for` loop uses different mechanisms depending on the type:

| Syntax | Trait | Desugaring | Notes |
|--------|-------|------------|-------|
| `for x in &coll` | `IndexIterator` | IndexIterator while loop with `get(i)` | Random-access collections |
| `for x in &mut coll` | `IndexIterator` | IndexIterator while loop with `get_mut(i)` | Random-access collections |
| `for x in coll` | `Iterator` / `IntoIterator` | While loop with `next()` | Generators, ranges, consuming |

**Why two mechanisms?**

- **IndexIterator (random-access)**: Enables reference iteration (`for x in &collection`) using the `get(&self, i): &T` method. Intersection semantics make this safe—each `get()` call returns a reference tied to the collection's lifetime. Requires O(1) indexed access.
...
## 2. The IndexIterator Trait

Types that support reference iteration via `for x in &collection` implement `IndexIterator`. This trait requires random/indexed access (O(1) element retrieval by index).
...
The name emphasizes both **indexed access** and **iteration**: types implementing `IndexIterator` support efficient O(1) random access by index, enabling index-based iteration via `get(i)` calls. This distinguishes it from the `Index` trait (which provides the `[]` subscript operator) and from `Iterator` (which yields owned values via `next()`).

Types without indexed access (hashmaps, linked lists, trees) should not implement `IndexIterator`. Instead, they use:
...
```spl
impl IndexIterator for Vec(T: T) where T {
    type Item = T;
...
impl IndexIterator for [T; N] where T {
    type Item = T;
...
Ranges implement `Iterator` rather than `IndexIterator` because they produce computed values rather than references to stored data:
...
impl Iter(S: S) where S: IndexIterator {
    /// Transform elements
    ...
    fn flatten(self): Flatten(S: Self) where S.Item: IndexIterator {
        Flatten(source: self)
    }

    /// Chain with another iterable
    fn chain(self, other: O): Chain(A: Self, B: O) where O: IndexIterator(Item: S.Item) {
        Chain(first: self, second: other)
    }

    /// Zip with another iterable
    fn zip(self, other: O): Zip(A: Self, B: O) where O: IndexIterator {
...
> **Note:** Generators are consumed during iteration. Unlike indexable collections, generators maintain internal state and produce values on-demand. The `for` loop over a generator uses the `Iterator` trait (with `next()` returning owned values), not `IndexIterator`. See "Consuming Iteration" below for the `Iterator` trait.
...
Non-indexed collections cannot implement `IndexIterator` because:
- `IndexIterator` requires O(1) random access via `get(&self, i): &T`
...
|-----------|---------------|------------|----------|
| `for x in &coll` (IndexIterator) | Yes (scoped) | No | Random-access iteration by reference |
...
| Trait | Purpose |
|-------|---------|
| `IndexIterator` | Random-access collections with `get(&self, i): &T` for reference iteration |
...
```
IndexIterator          - Random-access reference iteration (Vec, arrays)
    ↓ provides
...
2. **`for` loops use IndexIterator for random-access**: Desugars to `get()`/`get_mut()` calls
...
impl IndexIterator for CircularBuffer(T: T) where T {
    type Item = T;
...
- [traits.md](traits.md) - IndexIterator, Iterator, and RefIterator traits
